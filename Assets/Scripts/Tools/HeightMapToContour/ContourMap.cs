...
public class ContourMap
{
    ...
    public static Texture2D FromRawHeightmap16bpp(string fileName, int width = 0, int height = 0,int heightBandCount = 12)
    {
        ...
        int bandDistance = maxHeight / heightBandCount; //Number of height bands to create
        ...
        for (int b = 0; b < bands.Count; b++)
        {
            ...
            float h = (float)b / heightBandCount;
            h = h * 0.5f + 0.5f; // 0-1 -> 0.5 - 1
            byte c = (byte)(h * 255);
            ...
            for (int y = 1; y < _height - 1; y++)
            {
                for (int x = 1; x < _width - 1; x++)
                {
                    ...
                    {
                        // if any neibour's height lower than this one,this one is the contour line
                        if (slice[y * _width + (x - 1)] == false || slice[y * _width + (x + 1)] == false || slice[(y - 1) * _width + x] == false || slice[(y + 1) * _width + x] == false)
                        {
                            Color32 color = new Color32(0, c, 0, 1);
                            topoMap.SetPixel(x, y, color);
                        }
                    }
                }
            }
