#include "Assets/Scripts/ShaderInc/ProceduralModeling.cginc"
...
void Initialize(inout RayMarchData R)
{
}
...
{ 
	...
	float bump = 0.0;
	if (_BumpHeight != 0.0) {
		float r = simpleRand(floor((p.BUMP_PLANE) / _GridSize + _ObjectID * 0.057234)).x;
...
void PostEffect(inout GBufferOut O, VsOut I, RayMarchData R)
{
}

#include "Assets/Scripts/ShaderInc/RayMarchFramework.cginc"