...
public class Scattering : MonoBehaviour
{
    ...
    [Tooltip("Physical properties of the fog")]
    ScatterSettings _setting;

    [SerializeField]
    ScatterSettings _blendToSetting;

    [SerializeField] [Range(0.0f, 1.0f)] float _blendTime;

    public float BlendTime
    {
        get => _blendTime;
        set => _blendTime = Mathf.Clamp01(value);
    }

    public ScatterSettings Setting
    {
        get
        {
            if (_setting == null)
            {
                _setting = s_defaultSetting;
            }
    ...
    public static ScatterSettings s_defaultSetting => Resources.Load<ScatterSettings>("DefaultLightScatterSetting");

    /// <summary>
    ///     Target blend _setting. Control blending with <see cref="BlendTime" />
    /// </summary>
    public ScatterSettings BlendToSetting
    {
        get
        {
            if (_blendToSetting == null)
            {
                _blendToSetting = s_defaultSetting;
            }

            return _blendToSetting;
        }
    }
    ...
    /// <summary>
    ///     Whether or not Forward shaders have the proper Scattering integration. Disable to revert to using a fullscreen pass to
    ///     apply Scattering
    /// </summary>
    ...
    [NonSerialized] internal ScatterKernel LightSpotKernel;
    [NonSerialized] internal ScatterKernel LightDirKernel;
    ...
    void OnEnable()
    {
        ...
        m_vaporCompute = Resources.Load<ComputeShader>("ScatterCompute");
        ...
        m_fogMat = new Material(Shader.Find("Hidden/ScatterPost")) { hideFlags = HideFlags.HideAndDontSave };
    ...
    void Update()
    {
        ...
        while (ScatterObject.s_objList.Count >= m_spheres.Length)
        {
            Array.Resize(ref m_spheres, m_spheres.Length * 2);
        }

        for (int i = 0; i < ScatterObject.s_objList.Count; i++)
        {
            var vap = ScatterObject.s_objList[i];
    ...
    void OnPreRender()
    {
        ...
        m_cullGroup.SetBoundingSphereCount(ScatterObject.s_objList.Count);
    ...
    internal void InjectObject(Matrix4x4 viewProj, int kernel, ScatterObject obj)
    {
        if (kernel == -1)
        {
            return;
    ...
    void DoComputeSteps()
    {
        ...
        Setting.Bind(m_vaporCompute, m_densityKernel, BlendToSetting, _blendTime);
        ...
        Profiler.BeginSample("Scattering Object Passes");
        //If there's no directional -> manual clear light buffer
        if (ScatterObject.s_objList.Count == 0 || ScatterObject.s_objList[0] as ScatterLight == null ||
            ((ScatterLight)ScatterObject.s_objList[0]).LightType != LightType.Directional)
        {
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        if (IsEditorCamera && !DisplayInSceneView)
        {
            Profiler.BeginSample("Scattering Direct Blit");
